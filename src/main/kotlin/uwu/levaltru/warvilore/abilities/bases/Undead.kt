...
abstract class Undead(string: String) : AbilitiesCore(string) {
    ...
    override fun onTick(event: ServerTickEndEvent) {
        if (player!!.health > player!!.getAttribute(Attribute.GENERIC_MAX_HEALTH)!!.value - 1.5
            && player!!.activePotionEffects.map { it.type }.contains(PotionEffectType.REGENERATION)) {
            player!!.damage(0.01)
            player!!.health -= 1.5
        }
        if (player!!.health < 1.5
            && player!!.activePotionEffects.map { it.type }.contains(PotionEffectType.POISON))
